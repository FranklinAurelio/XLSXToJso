...
/**
 * Analisar as configurações de todas as planilhas na pasta de trabalhoAnalisar as configurações de todas as planilhas na pasta de trabalho
 * @param {*} workbook 
 */
function parseSettings(workbook) {
  ...
  workbook.forEach(sheet => {
    ...
    //A folha que começa com um ponto de exclamação não é impressa
...
/**
 * Analisar uma tabela (folha)
 *
 * @param sheet Dados brutos da tabela
 * @param setting Configuração da tabela
...
function parseSheet(sheet, setting) {
  ...
  for (let i_row = headIndex; i_row < sheet.data.length; i_row++) {
    ...
    if (setting.type === SheetType.MASTER) {
      ...
      if (!id_cell) {
        throw `Na planilha ${sheet.name} Não é possível obter a coluna id`;
      }
  ...
  for (let index = 0; index < head.length; index++) {
    ...
    switch (type) {
      ...
      case DataType.ARRAY:
        ...
        break;
      default:
        console.log('Tipo não reconhecido:', '[' + rowIndex + ',' + index + ']', cell, typeof (cell));
        break;
    }
  }
    ...
    for (let name in settings) {
      if (settings[name].type === SheetType.MASTER) {
        ...
        settings[name].slaves.forEach(slave_name => {
          ...
          //slave Todos os dados na tabela
